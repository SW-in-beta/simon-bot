...
**simon-oncall** — 온콜 문의 분석 스킬. 슬랙 URL과 레포 이름을 받아 문의 맥락 파악 → 최신 코드 분석 → simon-study 심층 분석 → 원인/해결 방향 → 슬랙 답변 초안까지 제공합니다. ERROR, DATA_ANOMALY, LOGIC_INQUIRY, CAPABILITY 4가지 문의 유형을 분류하고 유형별 적합한 톤으로 답변을 생성합니다.
