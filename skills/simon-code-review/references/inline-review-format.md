...
**1. 직접 진술 — 은유·차용 용어 금지.** 데이터 구조·흐름을 다른 도메인 용어로 압축하지 않는다. "LEFT JOIN 시맨틱", "fan-in 가능", "INFILE 관점" 같은 표현은 그 도메인에 익숙한 독자에게만 통하고, 실제로 무엇이 어떻게 되는지는 리뷰어가 여전히 추론해야 한다 — 실제로 이 표현들이 "친숙하지 않습니다, 이해하기 쉽게 작성해주세요"를 받은 사례가 있다. 평문으로 그대로 서술한다. 업계 표준 명칭(N+1 쿼리, race condition 등)은 은유가 아니므로 대상이 아니다. (상세: `simon-dev/references/generation-style-guide.md`의 "직접 진술 우선")

**2. 낯선 개념은 1줄 정의를 먼저 붙인다.** 리뷰 포인트에서 언급하는 언어·런타임·DB 원시 기능(`atomic.Pointer`, `sync.Map`, `SRANDMEMBER`, `LRANGE`, `rand.Perm`, 프레임워크 고유 개념 등) 중 **이 레포에 선례가 0건인 것**은 정의 없이 쓰지 않는다. 형식: `{기능명}: {일반적 정의 1줄} — 여기서는 {이 코드에서의 구체적 역할}`.

판정은 별도 프로필 파일 없이 **Phase B의 기존 구현 참조(P-013) 결과를 재사용**한다 — `implementation.md`의 "선례 조사" 섹션에서 "코드베이스 최초 도입"으로 기록된 항목이 그 목록이다. 레포에 선례가 없으면 리뷰어에게도 낯설 가능성이 높다는 대리 신호이며, 매 리뷰마다 재평가되므로 오래된 프로필이 어긋날 위험이 없다.

**3. 결론 문장으로 닫는다.** 메커니즘 설명만으로는 "이해는 됐는데, 이렇게 함으로써 얻는 이점을 모르겠습니다"라는 재질문이 온다. 리뷰어가 실제로 걱정한 지점에 명시적으로 귀결하는 1문장을 붙인다: `따라서 이 변경은 {리뷰어가 우려한 지점}에 {영향/무영향}입니다.`
...
**동작 예시**: {아래 트리거 중 하나에 해당하면 필수, 아니면 생략 — 상세는 "동작 예시 트리거" 절}
...
아래 중 하나에 해당하면 **실제 값이 담긴 예시**를 넣는다. 제어흐름 서술("elif 체인 순서: A → B → default")은 예시가 아니다 — 리뷰어가 값을 상상해야 하는 서술은 "예시를 들어 설명해주세요"라는 되물음을 만든다.

| 트리거 | 넣을 것 | 예 |
|---|---|---|
| 캐시·DB 키 구조를 신규 정의·변경 | 실제 키 문자열 1개를 값까지 채워서 | `best_product:1052` → `SET {"A0001", "A0002", …}` (3개 저장 시) |
| 값이 여러 메서드·단계를 거쳐 조립됨 | 입력값 하나를 골라 단계별 중간값을 순서대로 | `"w:"` → 저장소 상수 흡수 → `"w:"` → 클라이언트 등록 문자열 `["w:", "ab:", …]` |
| 반환 데이터 구조가 신규이거나 필드 관계가 자명하지 않음 | 실제 반환값 예시 1개 (필드별 출처 포함) | `{"action": "NEW", "crawl_urls": ["…"], "schedule": null}` — schedule은 워크플로 values에 없으면 null |

전 변경 단위에 강제하면 코멘트가 비대해지므로 위 트리거에 한정한다.
...
2. {위와 동일}
3. {위와 동일}
**동작 예시**: {위와 동일 — "동작 예시 트리거" 절의 조건에 해당하면 필수}
