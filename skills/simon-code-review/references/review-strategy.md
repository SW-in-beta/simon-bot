...
4-B. **History-First Check**: diff에서 기존 코드를 **제거하거나 단순화하는 변경**이 있으면,
   해당 코드가 추가된 커밋의 맥락을 반드시 확인한다.
   - **적용 대상**: 검증 로직 제거, 방어 코드 삭제, 중복으로 판단된 로직 합치기,
     "불필요해 보이는" 분기 삭제, 하드코딩된 값/기본값 제거
   - **확인 방법**:
     ```bash
     git log --follow -p -- {file_path} | grep -A 20 "^commit" | head -60
     # 또는 해당 라인의 커밋 특정 시
     git log -S "{코드_스니펫}" --oneline -- {file_path}
     git show {commit_hash}
     ```
   - **판단 기준**: 커밋 메시지가 "fix:", "hotfix:", "revert:", "restore:" 등으로 시작하거나,
     PR/이슈 번호가 연결되어 있으면 → 장애 대응 또는 의도적 보완책일 가능성이 높다.
     이 경우 제거 전에 인라인 코멘트에 `[HISTORY-CONTEXT: {커밋 요약}]`을 명시하고
     severity를 최소 MEDIUM으로 판정한다.
   - **비용 절감**: 변경이 100줄 이상인 대규모 PR에서는 제거 라인이 10줄 이상인 청크만 적용한다.
...
1. **SQL & Data Safety**: raw SQL 사용 여부, 마이그레이션 롤백 가능성, 데이터 손실 위험. `strings.ReplaceAll` / `fmt.Sprintf` / 문자열 연결로 외부 입력을 쿼리에 삽입하는 패턴 포함. exported 함수의 파라미터가 쿼리 삽입 대상이면 MEDIUM 이상으로 플래그 (현재 caller가 안전하더라도 인터페이스 계약 관점에서 검증). Athena/BigQuery 등 비표준 SQL 환경도 동일 적용.
