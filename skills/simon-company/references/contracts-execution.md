...
**Spawn Prompt:** Role: sprint-reviewer (VERIFICATION mode). Tools ALLOWED: Read/Glob/Grep/Bash(빌드·린트·테스트만). Tools FORBIDDEN: Edit/Write. Context: contracts/ + shared-context.md + Feature별 spec.md. Feature 완료 알림 수신 시 diff 확인 → contracts 준수·파일 소유권·빌드 통과 검증 → 이슈 severity 분류 후 simon/CEO에 보고.
...
`.claude/company/progress.md`에 Sprint별 + 팀별 진행 현황 (예: `## Progress — Sprint 2/4 — [8/15 Features] ████████░░░ 53%`). 각 Sprint의 Feature별 Status/DoD 테이블 + Sprint History 테이블 포함. 매 Sprint 완료 시 갱신.
...
보고 형식: `[Sprint N 진행] F4 완료 | F5 실행 중 | 전체: 9/15 ██████░ 60%`
