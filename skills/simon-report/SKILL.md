...
- 분석 주제: {주제 요약}
- 분석 범위: {자동 탐색|특정 경로}
- 대상 독자: {개발자 내부 문서|비개발자 포함 공식 문서|경영 보고} — {추론 근거}
- 게시 플랫폼: {Confluence|Notion|슬랙|이메일|로컬 전용} — {추론 근거}
- 변경하려면 알려주세요.
```

추론 기준: "팀 내", "내부" → 개발자 내부 문서; "공유", "올려줘", "보고" → 비개발자 포함 또는 경영 보고. 플랫폼 불명확 시 Confluence를 기본값으로 추론.
...
- **Feature Scope 추출 (조건부 — 분석 주제가 특정 기능/피처일 때)**:
  - 기능의 entry condition (라인아이템 타입/전략/유저 조건 등 적용 대상)
  - 전제 조건과 예외 케이스
  - 코드/문서에서 확인되지 않으면 `[미확인]` 태그로 명시
  - 이후 커버리지·영향 범위 분석의 모집단 정의는 반드시 entry condition과 일치해야 함 — "전체 X 중 Y%만 Z" 결론을 내리기 전 X의 정의가 entry condition과 일치하는지 재검증
...
- Spawn `writer` (model: sonnet):
  ...
  - **독자 독립성 체크 (Reader Independence Check):**
    - writer는 self-audit 완료 후 추가로 다음을 확인한다. 기준은 "이 보고서를 세션에 참여하지 않은 독자가 처음 읽었을 때 이해 가능한가?"이다
    - **세션 내 의사결정 맥락** — "이번 분석에서는 ~만 사용", "2가지 뿐이다", "기존에 논의한 대로" 같은 표현이 있으면, 범위 축소·지표 선택·접근법 선택 등 세션 중 이루어진 **결정 자체가 아닌 결정의 이유**가 문서에 담기도록 교체한다
    - **편집 이력 흔적** — "수정 요청했던 부분", "앞서 언급했듯이", "이전 버전에서 바꾼" 같은 편집 과정 흔적은 삭제하거나 독립적 문장으로 재기술한다
    - **비교 맥락** — "기존 대비 줄임", "원래 ~였으나" 같은 상대적 표현은 현재 상태와 그 이유를 담은 절대적(standalone) 표현으로 교체한다
  - **공식 문서 절대 원칙 체크 (Publication Readiness Check):**
    - 기준: Step 0에서 확인한 대상 독자와 게시 플랫폼을 기준으로 아래를 검증한다.
    - **Glossary-First**: 본문에 처음 등장하는 전문 용어(기술 약어, 도메인 용어, 지표명 등)가 문서 앞부분(§1 또는 별도 용어 정의 섹션)에서 먼저 정의되어 있는지 확인한다. 본문 첫 등장이 정의보다 앞서면, 정의 섹션으로 이동시키거나 첫 등장 위치에 인라인 정의(괄호 내 한 줄 설명)를 추가한다
    - **고유명사 추상화**: 사내 서비스명(biddersvc, adpacingsvc 등), 내부 시스템 코드명, 광고주명, 팀명 등 비개발자·외부 독자가 이해하기 어려운 고유명사를 일반 용어로 대체하거나, 첫 등장 시 '일반 용어(고유명사)' 형식으로 병기한다. 대상 독자가 '개발자 내부 문서'이면 이 항목을 skip할 수 있다. 단, 사내 모든 팀이 동일한 단어로 부르는 도메인 핵심 용어(예: lineitem)는 풀어 쓰지 않고 그대로 둔다
    - **플랫폼 독립적 출처**: 로컬 파일 경로(`~/claude-reports/...`, `~/Obsidian/...`, Redash CSV 경로 등)를 근거 또는 링크로 인용하지 않는다. 출처가 필요하면 Confluence URL, Slack permalink, 공개 URL, 또는 '내부 코드베이스 분석 결과 (file:line)' 형식으로 대체한다. 어느 플랫폼(Confluence, Notion 등)에 게시해도 이상하지 않은 형식이어야 한다
