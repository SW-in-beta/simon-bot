...
- 테스트 가능성 (DI, 인터페이스 분리)
- SOLID/DRY 원칙 준수
- 과잉설계 / Economy of Means: 단일 구현뿐인 추상화·인터페이스·팩토리, 불필요한 레이어·config, 네이티브 기능(예: `<input type="date">`)으로 될 것의 커스텀 구현 여부. 기존 코드/표준 라이브러리/네이티브로 동일 목적을 더 적은 코드로 달성 가능했는가 (단, 테스트·검증·에러처리 코드는 과잉설계 판정 대상 아님 — 완전성 관할)
