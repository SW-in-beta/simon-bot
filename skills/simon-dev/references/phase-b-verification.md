...
- `.claude/memory/expert-plan-concerns.md` 읽기 (Phase A-4의 `## Historical Context` 섹션 포함)
...
- `[HISTORICAL-MATCH]` 태깅된 concern은 **trigger_condition이 부분 충족되어도 우선 검토** — 과거 실사건 기반이므로 재발 비용이 높다
- Spawn `architect`: 사전 우려사항 중 누락 항목 대조
- 누락 발견 → executor fix → 7-A 재검증 (max 1회)

#### Historical Incident Cross-Reference (Brain)

7-A findings가 확정된 후, Phase A에서 저장한 `.claude/memory/brain-historical-context.md`의 `## 과거 우려/인시던트` 섹션과 **7-A findings**를 직접 대조한다. 7-A는 blind로 수행되므로(Blind-First Protocol), 이 교차 확인은 오직 비(非)blind 단계인 7-B에서만 수행한다 — fresh subagent(verifier, devil-advocate, alignment-checker)에는 brain 결과를 절대 전달하지 않는다.

- Brain 파일이 없거나 "해당 없음"이면 이 단계 skip
- 7-A findings와 brain 항목 매칭:
  - **[HISTORICAL-CONFIRM]**: 7-A가 독립적으로 발견 + brain에도 과거 동일/유사 인시던트 기록 → 신뢰도 최고 등급. executor 우선순위 상향 + Severity-First Ordering에서 최상단 배치
  - **[HISTORICAL-MISS]**: brain에만 있고 7-A에서 놓친 항목 → 해당 코드 위치를 verifier에 **코드 위치만** 전달하여 blind 재검증 (brain 맥락은 여전히 비공개). verifier가 이슈를 재현하면 finding으로 추가, 재현 실패 시 `[UNVERIFIED — historical-only]` 태깅하여 MEDIUM으로 기록만
  - **[OBSOLETE]**: 현재 코드 구조에서 더 이상 해당하지 않음 → 무시하고 brain 파일에 `(obsolete)` 태깅
- `high_impact_paths` 매칭 파일에 `[HISTORICAL-MISS]`가 발견되면 `devil_advocate_required=true` 플래그를 설정하여 Devil's Advocate 단계에서 집중 탐색하도록 지시 (단, 힌트 텍스트는 비공개, 플래그만 전달)

**출력**: `review-findings.md`의 각 finding에 해당 시 `historical_reference` 필드 추가 (brain 문서 경로). 신규 `[HISTORICAL-MISS]` findings는 별도 섹션 `## Historical Misses`로 분리.
...
**Subagent 위임 원칙**: importers grep 및 영향 파일 분석은 subagent에 위임하고, "영향 파일 목록 + 조치 필요 여부" 결론만 반환받는다. grep 출력 원문이 메인 컨텍스트에 축적되는 것을 방지한다 (SKILL.md의 Subagent Result-Only 반환 원칙 참조).
...
### Side Effect Check (Step 11 — 호출 체인 탐색)

**Subagent 위임 원칙**: caller 체인 탐색(Grep + Read)은 subagent에 위임하고, "확인된 side effect 목록 + 조치 필요 여부" 결론만 반환받는다 (SKILL.md의 Subagent Result-Only 반환 원칙 참조).
