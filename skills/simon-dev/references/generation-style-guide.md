...
형식:
```
[Recommendation] A) STANDARD — 단일 모듈 변경, 전체 품질 파이프라인으로 충분 (Completeness: 8/10)
[Not Recommended] B) LARGE — 단일 모듈 범위, extended failure mode 분석은 오버엔지니어링
```
...
단, 이 원칙은 *요구사항 커버리지*에 적용되지 *코드량 자체*에 적용되는 것이 아니다. 같은 요구사항(같은 AC·엣지·에러 커버리지)을 더 적은 코드로 달성할 수 있는데 굳이 쓴 70줄이라면, 그것은 품질이 아니라 부채다 — 읽을 표면과 깨질 표면이 늘어난다. 완전성(이 섹션)은 "무엇을 구현하는가"를, 절제(다음 섹션 Economy of Means)는 "어떻게 구현하는가"를 관장한다. 둘은 충돌이 아니라 직교한다: **모든 요구사항을 100% 커버하되, 그 커버리지를 가장 적은 코드로 달성한다.**
...
Section 4(완전성)가 "무엇을 구현하는가"를 100% 채우라고 한다면, 이 원칙은 "어떻게 구현하는가"를 가장 적은 코드로 달성하라고 한다. **더 많은 코드가 더 완전한 것이 아니다 — 같은 요구사항을 더 적은 코드로 충족하면 버그 표면이 줄어 더 안정적이다.**

코드를 작성하기 전 사다리를 탄다(문제를 완전히 이해한 *후*): 존재 필요?(YAGNI) → 이미 코드베이스에 있나?(재사용) → 표준 라이브러리 → 네이티브 플랫폼 기능 → 설치된 의존성 → 한 줄 → 그제서야 최소 코드.

### 절제의 대상 (과잉설계)

| 과잉설계 | 대안 |
|---------|------|
| 구현 하나뿐인 인터페이스 / 제품 하나뿐인 팩토리 | 직접 호출 |
| 네이티브로 될 것을 커스텀 컴포넌트로 (date/color/file picker) | 네이티브 기능 |
| `lru_cache` 한 줄로 될 일에 캐시 클래스 | 표준 라이브러리 |
| "나중을 위한" 스캐폴딩·추상 레이어 | 그때 만든다 |

### 경계 (Section 4와의 관계 — 반드시 지킨다)

절제는 코드의 *형태*에만 적용된다. 아래는 절대 줄이지 않는다 (Section 4 완전성 관할):
- **테스트** — TDD 예외 없음. "사소하니 테스트 불필요"는 이 원칙에 포함되지 **않는다**.
- **요구사항·엣지·에러 커버리지** — "shortest diff"를 목표로 삼지 않는다. 목표는 "요구사항을 완전히 커버하는 최소 코드".
- **검증·에러처리·보안·접근성** — 단순화 대상 아님.
- **기존 코드 삭제·단순화** — Intent-Before-Fix Gate(git blame) 통과 필수.

상세 anti-pattern과 `lazy:` 주석 규약은 [`~/.claude/skills/_shared/lazy-output-discipline.md`](../../_shared/lazy-output-discipline.md)를 참조한다.
