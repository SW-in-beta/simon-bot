...
**독자 독립성 검증 (게시 전 최종 체크):**

형식 변환 완료 후, 내용 수준에서 다음을 확인한다. 기준은 "이 문서를 세션에 참여하지 않은 독자가 처음 읽었을 때 이해 가능한가?"이다:

- **세션 맥락 레퍼런스**: "이번 분석에서는 ~만", "2가지 뿐이다", "기존에 논의한 대로" 같은 표현이 있으면, 범위·지표·결정의 **이유**가 문서 내에서 자립적으로 설명되는지 확인한다
- **편집 이력 흔적**: "수정 요청했던", "앞서 언급했듯이", "이전 버전에서 바꾼" 같은 수정 과정 언급이 본문에 남아 있지 않은지 확인한다
- **비교 맥락**: "기존 대비 줄임", "원래 ~였으나" 같은 상대적 표현이 절대적(standalone) 표현으로 정제되었는지 확인한다

문제 발견 시 게시 전 사용자에게 보고하고 수정 여부를 확인한다.
